...
**设计理念**：
AI 引导学生自我诊断错误，通过三问复盘形成深度记忆，而非直接告诉答案。培养学生独立分析问题的能力。

**核心流程**：
```
提交失败 → 记录错题 → AI分析错误类型 → 三问复盘 → 生成防错规则
```
...
│  📓 错题本                                    [防错规则]     │
├─────────────────────────────────────────────────────────────┤
│  统计卡片：总错题数 | 待复盘 | 复盘中 | 已完成              │
├─────────────────────────────────────────────────────────────┤
│  错误类型分布（10种）                                        │
│  答案错误[WA]: 📖读错题(2) 🔲边界漏(3) 🧩逻辑错(1) ...     │
│  其他状态: ✏️语法错(1) 🐢超时了(0) ...                      │
├─────────────────────────────────────────────────────────────┤
│  筛选：[状态▼] [错误类型▼]                                   │
│                                                             │
...
防错规则页 `/error-book/rules`：
```
┌─────────────────────────────────────────────────────────────┐
│  🛡️ 防错规则                                   ← 返回错题本 │
├─────────────────────────────────────────────────────────────┤
│  统计：总规则数(12) | 启用中(10) | 总触发次数(25)           │
├─────────────────────────────────────────────────────────────┤
│  ⚠️ 需要注意的规则（高频触发）                               │
│  ┌─────────────────────────────────────────────────────────┐│
│  │ 🔲 写循环前，先确定边界 i=0 还是 i=1      触发 8 次   ││
│  │ 📖 读完题后，在草稿纸上列出所有特殊情况    触发 5 次   ││
│  └─────────────────────────────────────────────────────────┘│
├─────────────────────────────────────────────────────────────┤
│  按类型筛选：[全部类型▼]                                     │
│                                                             │
...
model ErrorCase {
  ...
  userId        String
  submissionId  String   @unique  // 关联失败提交
  problemId     String

  errorType     String?  // 10种错误类型之一
  q1Answer      String?  @db.Text  // 第一问答案
  q2Answer      String?  @db.Text  // 第二问答案
  q3Answer      String?  @db.Text  // 第三问答案
  aiAnalysis    Json?    // AI分析结果

  preventionRuleId String?  // 关联的防错规则
...
model PreventionRule {
  ...
  rule          String   @db.Text  // 规则内容
  examples      String[] // 触发案例列表
...
1. 错误分类提示词：分析代码和判题结果，返回10种错误类型之一
...
| 错题本列表 | `/error-book` | 错题列表、按类型筛选 | ✅ |
| 错题复盘 | `/error-book/[id]` | 三问复盘流程 | ✅ |
| 防错规则 | `/error-book/rules` | 防错规则管理 | ✅ |
| 数据导入 | `/import` | 导入洛谷数据 | 🚧 |
...
| 方法 | 端点 | 功能 |
|------|------|------|
| GET | `/api/error-case` | 获取用户错题列表（支持筛选） |
| POST | `/api/error-case` | 从失败提交创建错题记录 |
| GET | `/api/error-case/[id]` | 获取错题详情 |
| PATCH | `/api/error-case/[id]` | 更新错题（提交三问答案） |
| DELETE | `/api/error-case/[id]` | 删除错题记录 |
| POST | `/api/error-case/[id]/analyze` | AI分析错误类型 |
| POST | `/api/error-case/[id]/hint` | AI三问引导提示 |
| GET | `/api/prevention-rules` | 获取用户防错规则 |
| POST | `/api/prevention-rules` | 创建防错规则 |
| GET | `/api/prevention-rules/[id]` | 获取单条规则详情 |
| PATCH | `/api/prevention-rules/[id]` | 更新规则（启用/停用） |
| DELETE | `/api/prevention-rules/[id]` | 删除规则 |
| POST | `/api/prevention-rules/check` | 检查代码是否触发已有规则 |
