...
const gesp5Problems = [
  // ========== 样题 ==========
  {
    ...
    description: `班级有 $n$ 名同学，第 $i$ 位同学每隔 $a_i$ 天锻炼一次，也就是说，如果第 $i$ 位同学某天锻炼了，那么他下次锻炼的时间是 $a_i$ 天后。

某天，$n$ 位同学恰好同时来锻炼了，他们想知道，下次全部同学一起来锻炼，还需要过多少天。`,
    inputFormat: `第一行包含一个整数 $n$，表示同学数量。

第二行包含 $n$ 个正整数 $a_0, a_1, \\ldots, a_{n-1}$，相邻整数之间用空格隔开。`,
    outputFormat: `输出一个整数，表示下次所有同学都来锻炼需要过的天数。`,
    samples: [
      { input: "3\n1 2 3", output: "6", explanation: "第一位同学每天都锻炼；第二位每 2 天一次；第三位每 3 天一次。6 天后三人一起锻炼，之前不可能三人同时锻炼。" },
      { input: "4\n2 4 8 16", output: "16", explanation: "第四位同学每 16 天锻炼一次，恰好与其他同学锻炼日期重合。" },
      { input: "4\n2 4 6 8", output: "24" },
    ],
    testCases: [
      { input: "3\n1 2 3", output: "6" },
...
接下来 $M$ 行，每行一个整数 $x$（$0 \\le x < N$），表示点名学号为 $x$ 的同学。`,
    ...
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于所有测试点，$1 \\le M \\le N \\le 2000$。
...
对于 $50\\%$ 的测试点，所有同学身高互不相同。`,
  ...
  // ========== 2023年9月 ==========
  {
    ...
    description: `每个正整数都可以分解成素数的乘积，例如：$6=2\\times 3$，$20=2^2\\times5$。现在，给定一个正整数，请按要求输出它的因数分解式。`,
    inputFormat: `输入第一行，包含一个正整数 $N$。约定 $2 \\le N \\le 10^{12}$。`,
    outputFormat: `输出一行，为 $N$ 的因数分解式。要求按质因数由小到大排列，乘号用星号 \`*\` 表示，且左右各空一格。当且仅当一个素数出现多次时，将它们合并为指数形式，用上箭头 \`^\` 表示，且左右不空格。`,
    samples: [
      { input: "6", output: "2 * 3" },
...
1. 游戏分为 $n$ 个时间段，每个时间段可选择一个小游戏参加。
2. 共有 $n$ 个小游戏可供选择。
3. 第 $i$ 个小游戏必须在第 $T_i$ 个时间段结束前完成才能获得奖励 $R_i$。

小明足够聪明，每个小游戏在一个时间段内都能完成。问题：如何安排每个时间段的选择，使总奖励最大化？`,
...
第三行包含 $n$ 个正整数 $R_i$，表示第 $i$ 个小游戏的奖励，$1 \\le R_i \\le 1000$。`,
    ...
    samples: [
      { input: "7\n4 2 4 3 1 4 6\n70 60 50 40 30 20 10", output: "230", explanation: "7 个时间段分别完成第 4、2、3、1、6、7、5 个小游戏。其中第 4、2、3、1、7 个小游戏在期限内完成，获得奖励 40+60+50+70+10=230。" },
    ],
    testCases: [
...
- 否则输出幸运化后的结果`,
    ...
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $30\\%$ 的测试点，$a, x \\le 100$，$N \\le 100$。
...
接下来一行 $N$ 个用空格隔开的整数，依次为 $a_1,\\cdots,a_N$，表示各种食材的美味度。`,
    ...
    samples: [
      { input: "3\n1 2 3", output: "2", explanation: "编号为 $2,3$ 的食材之间的契合度为 $2\\ \\text{and}\\ 3=2$，是所有食材两两之间最高的契合度。" },
      { input: "5\n5 6 2 10 13", output: "8", explanation: "编号为 $4,5$ 的食材之间的契合度为 $10\\ \\text{and}\\ 13=8$，是所有食材两两之间最高的契合度。" },
    ],
    testCases: [
      { input: "3\n1 2 3", output: "2" },
...
对于所有测试点，保证 $N \\le 10^6$，$0 \\le a_i \\le 2,147,483,647$。`,
  ...
  // ========== 2024年3月 ==========
  {
    ...
    description: `有 $n$ 名同学，每名同学有语文、数学、英语三科成绩，需按照规则从高到低排序：

1. 比较总分，高者靠前
2. 总分相同则比较语文和数学两科总分，高者靠前
3. 仍相同则比较语文和数学两科的最高分，高者靠前
4. 仍相同则二人并列

输出每位同学的排名。若 $x$ 人并列，则排名相同，留空后面 $x-1$ 个名次。`,
    inputFormat: `第一行一个整数 $N$（同学人数）。

接下来 $N$ 行，每行三个非负整数 $c_i, m_i, e_i$，分别表示该同学的语文、数学、英语成绩。`,
    outputFormat: `输出 $N$ 行，按输入同学的顺序，输出各自的排名。**注意：按同学的顺序输出排名，不按排名输出序号。**`,
    samples: [
      { input: "6\n140 140 150\n140 149 140\n148 141 140\n141 148 140\n145 145 139\n0 0 0", output: "1\n3\n4\n4\n2\n6" },
    ],
    testCases: [
    ...
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $30\\%$ 的数据，$N \\le 100$，所有同学总分各不相同。
...
对于全部数据，$2 \\le N \\le 10^4$，$0 \\le c_i, m_i, e_i \\le 150$。`,
  },
  {
    ...
    difficulty: "普及+/提高",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1145`,
    description: `小杨同学想寻找一种名为 B-smooth 数的正整数。如果一个正整数的最大质因子不超过 $B$，则该正整数为 B-smooth 数。
...
小杨同学想知道，对于给定的 $n$ 和 $B$，有多少个不超过 $n$ 的 B-smooth 数。`,
    ...
    samples: [
      { input: "10 3", output: "7", explanation: "在不超过 10 的正整数中，3-smooth 数有 {1,2,3,4,6,8,9}，共 7 个。" },
    ],
    testCases: [
      { input: "10 3", output: "7" },
    ],
    ...
    hint: `| 子任务 | 得分 | $n \\le$ | $B$ |
|:---:|:---:|:---:|:---:|
| 1 | 30 | $10^3$ | $1 \\le B \\le 10^3$ |
| 2 | 30 | $10^6$ | $\\sqrt{n} \\le B \\le 10^6$ |
| 3 | 40 | $10^6$ | $1 \\le B \\le 10^6$ |
...
之后 $n$ 行，每行一个长度为 $m$ 的 $\\texttt{01}$ 串，代表网格图第 $i$ 行格子的颜色，如果为 $\\texttt{0}$，则对应格子为白色，否则为黑色。`,
    ...
    samples: [
      { input: "4 5 5\n00000\n01111\n00011\n00011", output: "6", explanation: "至少包含 $5$ 个黑色格子的最小子矩形的四个顶点为 $(2,4)$，$(2,5)$，$(4,4)$，$(4,5)$，共包含 $6$ 个格子。" },
    ],
    testCases: [
      { input: "4 5 5\n00000\n01111\n00011\n00011", output: "6" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于全部数据，$1 \\le n,m \\le 100$，$1 \\le k \\le n \\times m$。
...
| 3 | 40 | $\\le 100$ |`,
  },
  {
    ...
    difficulty: "普及/提高-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1153`,
    description: `小杨定义幸运数字为恰好拥有两种不同质因子的正整数。例如，$12 = 2 \\times 2 \\times 3$，其质因子为 2 和 3（两种），故为幸运数字；而 $30 = 2 \\times 3 \\times 5$ 有三种质因子，不符合要求。

题目要求对 $n$ 个给定的正整数进行判断。`,
    inputFormat: `第一行包含一个正整数 $n$（代表正整数个数）。

之后 $n$ 行，每行一个正整数。`,
    outputFormat: `输出 $n$ 行，对于每个正整数，如果是幸运数字输出 $1$，否则输出 $0$。`,
    samples: [
      { input: "3\n7\n12\n30", output: "0\n1\n0", explanation: "7 的质因子仅有 7（一种）；12 的质因子为 2、3（两种）；30 的质因子为 2、3、5（三种）。" },
    ],
    testCases: [
      { input: "3\n7\n12\n30", output: "0\n1\n0" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于全部数据，$1 \\le n \\le 10^4$，$2 \\le a_i \\le 10^6$。
...
| 2 | 60% | $\\le 10^4$ | $\\le 10^6$ |`,
  ...
  // ========== 2024年9月 ==========
  {
    ...
    description: `小杨与怪物战斗，怪物血量为 $h$。仅当血量**恰好**为 $0$ 时才能击败。

两种攻击方式：
- **物理攻击**：第 $i$ 次使用造成 $2^{i-1}$ 点伤害
- **魔法攻击**：选择任意质数 $x$（不超过当前血量）造成 $x$ 点伤害，**至多一次**使用

求最少攻击次数，无法击败输出 $-1$。`,
    inputFormat: `第一行为正整数 $t$（测试用例组数）。

接下来 $t$ 行，每行一个整数 $h$（怪物血量）。`,
    outputFormat: `每组测试用例输出一个整数。若能击败，输出最少攻击次数；否则输出 $-1$。`,
    samples: [
      { input: "3\n6\n188\n9999", output: "2\n4\n-1", explanation: "第一组（$h=6$）：使用质数 $5$ 的魔法攻击（伤害5），再用第1次物理攻击（伤害1），共2次，血量恰好为0。" },
    ],
    testCases: [
      { input: "3\n6\n188\n9999", output: "2\n4\n-1" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `$1 \\le t \\le 10$，$1 \\le h \\le 10^5$。
...
| 3 | 60% | $\\le 10$ | $\\le 10^5$ |`,
  },
  {
    ...
    difficulty: "普及/提高-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1161`,
    description: `小杨有 $n$ 种不同的武器，初始熟练度为 $c_i$。参加 $m$ 场战斗，每场必须选一种武器。使用第 $i$ 种武器参加第 $j$ 场战斗，该武器熟练度从 $c'_i$ 变为 $c'_i + a_j$（$a_j$ 可正可负可为零）。

求如何选择武器使得 $m$ 场战斗后，$n$ 种武器的最大熟练度尽可能大。`,
...
第三行包含 $m$ 个整数 $a_1, a_2, \\ldots, a_m$（战斗熟练度变化值）。`,
    ...
    samples: [
      { input: "2 2\n9 9\n1 -1", output: "10", explanation: "最优方案：第一场战斗选择第一种武器，第二场战斗选择第二种武器。" },
    ],
    testCases: [
      { input: "2 2\n9 9\n1 -1", output: "10" },
    ],
...
总体约束：$1 \\le n, m \\le 10^5$，$-10^4 \\le c_i, a_i \\le 10^4$。`,
  },
  // ========== 2024年12月 ==========
  {
...
求这样的集合最多能包含多少个奇妙数字。`,
    ...
    samples: [
      { input: "128", output: "3", explanation: "一个包含 3 个奇妙数字的合法集合是 $\\{2,4,8\\}$。因为 $2=2^1$，$4=2^2$，$8=2^3$ 都是奇妙数字，且 $2 \\times 4 \\times 8=64$ 整除 128，答案为 3。" },
    ],
    testCases: [
      { input: "128", output: "3" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $100\\%$ 的测试数据：$2 \\le n \\le 10^{12}$。
...
之后 $m$ 行，每行包含两个正整数 $p_i,c_i$，代表第 $i$ 种强化材料的适配武器和修改花费。`,
    ...
    samples: [
      { input: "4 4\n1 1\n2 1\n3 1\n3 2", output: "1", explanation: "花费 $1$，将第三种强化材料的适配武器由 $3$ 改为 $1$。此时，武器 $1$ 有 $2$ 种强化材料适配，武器 $2$ 和武器 $3$ 都各有 $1$ 种强化材料适配，满足条件。" },
    ],
    testCases: [
      { input: "4 4\n1 1\n2 1\n3 1\n3 2", output: "1" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $100\\%$ 的数据：$1 \\le n,m \\le 1000$，$1 \\le p_i \\le n$，$1 \\le c_i \\le 10^9$。
...
| 3 | 60% | $\\le 1000$ | $\\le 1000$ |`,
  ...
  // ========== 2025年3月 ==========
  {
    ...
    description: `小 A 有 $2n$ 件物品，小 B 愿意以 $b_i$ 的价格购买第 $i$ 件物品，小 C 愿意以 $c_i$ 的价格购买第 $i$ 件物品。

为了公平起见，小 A 决定让小 B 和小 C 各购买恰好 $n$ 件物品。求小 A 的最大收入。`,
...
对于所有测试点，保证 $1\\le T\\le20$，$3\\le p\\le10^9$，$1<a<p$，$p$ 为质数。`,
  ...
  // ========== 2025年6月 ==========
  {
    ...
    description: `班主任向认真听讲和完成作业的学生分发课堂优秀券和作业优秀券。学生可用两种方式兑换奖品：
...
第二行包含两个正整数 $a,b$（兑换一份奖品所需的两种券数量）。`,
    ...
    timeLimit: 400,
    memoryLimit: 256,
    hint: `对于 $60\\%$ 的测试点，$1 \\le a,b \\le 100$，$1 \\le n,m \\le 500$。
...
- 选择序列中一个正整数 $A_i$（$1\\le i\\le N$），将 $A_i$ 变为 $A_i/P$，$P$ 为任意质数，要求 $A_i$ 是 $P$ 的倍数。
...
async function seedGesp5() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `成功导入 ${result.count} 道 GESP 5级题目（已更新为与洛谷完全一致）`,
      count: result.count
    });
  } catch (error) {
