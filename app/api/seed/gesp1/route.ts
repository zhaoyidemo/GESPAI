...
const gesp1Problems: SeedProblem[] = [
  // ========== 2023年3月 ==========
  {
...
第四行包含一个正整数 $Q$，是小明手里的钱数（单位：元）。`,
  ...
  },
  {
    ...
    testCases: [
      ...
      { input: "2\n1\n10", output: "30" },  // k=2：倍数2,4,6,8,10；个位2:2（已含）；去重后2+4+6+8+10=30
      { input: "5\n1\n20", output: "50" },  // k=5：倍数5,10,15,20；个位5:5,15（已含）；去重后5+10+15+20=50
      { input: "3\n1\n30", output: "201" },  // k=3：倍数3,6,9,12,15,18,21,24,27,30；个位3:3,13,23；去重后和=201
      { input: "9\n1\n100", output: "1026" },  // k=9：倍数9,18,...,99(和594)；个位9:9,19,...,99(和540)；重复9,99(和108)；594+540-108=1026
      { input: "2\n1\n1", output: "0" },  // 范围只有1，不是2的幸运数
      { input: "2\n2\n2", output: "2" },  // 范围只有2，是2的幸运数
      { input: "5\n100\n100", output: "100" },  // 100是5的倍数
      { input: "3\n999\n1000", output: "999" },  // 999是3的倍数，1000不是
      { input: "4\n10\n50", output: "348" },  // k=4：倍数12,16,20,24,28,32,36,40,44,48；个位4:14,24,34,44；去重后和=348
      { input: "6\n1\n60", output: "474" },  // k=6：倍数6,12,18,24,30,36,42,48,54,60；个位6:6,16,26,36,46,56；去重后和=474
    ],
    timeLimit: 1000,
    memoryLimit: 128,
    hint: `**样例解释 1**
...
对于样例 1，由于需要购买相同数量的两种商品，因此小杨最多能够购买 $4$  件商品 $A$ 和 $4$ 件商品 $B$，共花费 $1 \\times 4 + 2 \\times 4 = 12$ 元。因此，样例 1 的答案为 $4$。

### 样例 2 解释

对于样例 1，由于需要购买相同数量的两种商品，因此小杨最多能够购买 $4$  件商品 $A$ 和 $4$ 件商品 $B$，共花费 $1 \\times 4 + 2 \\times 4 = 12$ 元。因此，样例 1 的答案为 $4$。如果小杨想购买 $5$ 件商品 $A$ 和商品 $B$，则需要花费 $1 \\times 5 + 2 \\times 5 = 15$ 元，超过了小样的预算 $13$ 元。因此，样例 2 答案为 $4$。
...
export const { GET, POST } = createSeedHandler(gesp1Problems, "GESP 1级");
