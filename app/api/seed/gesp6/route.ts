...
$5$ 个亲朋数，分别 $10$、$102$、$0$、$02$、$2$。`,
  ...
  // ========== 2023年9月 ==========
  {
    ...
    description: `小杨来到了一家商店，打算购买一些饮料。这家商店总共出售 $N$ 种饮料，编号从 $0$ 至 $N-1$，其中编号为 $i$ 的饮料售价 $c\\_i$ 元，容量 $l\\_i$ 毫升。
...
接下来 $N$行，依次描述第 $i=0,1,\\cdots,N-1$ 种饮料：每行两个整数 $c\\_i,l\\_i$。`,
...
对于 $40\\%$ 的测试点，保证 $N \\le 20;1\\le L \\le 100; l\\_i \\le 100$。

对于 $70\\%$ 的测试点，保证 $l\\_i \\le 100$。

对于 $100\\%$ 的测试点，保证 $1\\le N \\le 500;1\\le L \\le 2000; 1\\le c\\_i,l\\_i \\le 10^6$。`,
  },
  {
...
这个游戏总共有 $N$ 关，每关都有 $M$ 个通道，你需要选择一个通道并通往后续关卡。其中，第 $i$ 个通道可以让你前进 $a\\_i$ 关，也就是说，如果你现在在第 $x$ 关，那么选择第 $i$ 个通道后，你将直接来到第 $x+a\\_i$ 关（特别地，如果 $x + a\\_i \\geq N$，那么你就通关了）。此外，当你顺利离开第 $s$ 关时，你还将获得 $b\\_s$ 分。

游戏开始时，你在第 $0$ 关。请问，你通关时最多能获得多少总分。`,
    inputFormat: `第一行两个整数 $N$，$M$，分别表示关卡数量和每关的通道数量。

接下来一行 $M$ 个用单个空格隔开的整数 $a\\_0,a\\_1\\cdots,a\\_{M-1}$。保证 $1\\le a\\_i \\le N$。

接下来一行 $N$ 个用单个空格隔开的整数 $b\\_0,b\\_1\\cdots,b\\_{N-1}$。保证 $|b\\_i|\\le 10^5$。`,
...
对于所有测试点，保证 $1 \\le N \\le 10^4$；保证 $1 \\le M\\le 100$。`,
  },
  {
    ...
    difficulty: "普及-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1138`,
    description: `某公司有 $N$ 名员工，编号从 $0$ 至 $N-1$。其中，除了 $0$ 号员工是老板，其余每名员工都有一个直接领导。我们假设编号为 $i$ 的员工的直接领导是 $f\\_i$。

该公司有严格的管理制度，每位员工只能受到本人或直接领导或间接领导的管理。具体来说，规定员工 $x$ 可以管理员工 $y$，当且仅当 $x=y$，或 $x=f\\_y$，或 $x$ 可以管理 $f\\_y$。特别地，$0$ 号员工老板只能自我管理，无法由其他任何员工管理。

现在，有一些同事要开展合作，他们希望找到一位同事来主持这场合作，这位同事必须能够管理参与合作的所有同事。如果有多名满足这一条件的员工，他们希望找到编号最大的员工。你能帮帮他们吗？`,
    inputFormat: `第一行一个整数 $N$，表示员工的数量。

第二行 $N - 1$ 个用空格隔开的正整数，依次为 $f\\_1,f\\_2,\\dots f_{N−1}$。
...
- 对全部的测试数据，保证 $1 \\leq a,b,c \\leq n \\leq 2 \\times 10^5$。`,
  },
  {
    ...
    difficulty: "普及-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1146`,
    description: `你有 $10^9$ 个牛棚，从左到右一字排开。你希望把 $n$ 头牛安置到牛棚里。麻烦的是，你的牛很好斗，如果他们附近有其他的牛，他们就会不安分地去挑事。其中，第 $i$ 头牛的攻击范围是 $(a\\_i, b\\_i)$，这意味着，如果他的左边 $a\\_i$ 个牛棚或者右边 $b\\_i$ 个牛棚有其他牛，它就会去挑事。
...
第三行 $n$ 个正整数 $b\\_1, b\\_2, \\dots b\\_n$。`,
    outputFormat: `输出一行一个整数表示答案。`,
    samples: [
      { input: "2\n1 2\n1 2", output: "4" },
      { input: "3\n1 2 3\n3 2 1", output: "7" },
...
- 另有 $20\\%$ 的数据，保证 $n = 3$。
- 对 $80\\%$ 的数据，保证 $n \\leq 8$。
- 对于所有的测试数据，保证 $1 \\leq n \\leq 9$，$1 \\leq a\\_i, b\\_i \\leq 10^3$。`,
  },
  // ========== 2024年6月 ==========
  {
...
小杨设置了一个包含 $n$ 个正整数的计分序列 $A=[a\\_1,a\\_2,\\ldots,a\\_n]$，如果字符串的一个子串由 $k(1\\leq k \\leq n)$ 个 $\\texttt{abc}$ 首尾相接组成，那么能够得到分数 $a\\_k$，并且字符串包含的字符不能够重复计算得分，整个字符串的得分是计分子串的总和。

例如，假设 ，字符串 $\\texttt{dabcabcabcabzabc}$ 的所有可能计分方式如下：
- $\\texttt{d+abc+abcabc+abz+abc}$ 或者 $\\texttt{d+abcabc+abc+abz+abc}$，其中 $\\texttt{d}$ 和 $\\texttt{abz}$ 不计算得分，总得分为 $a\\_1+a\\_2+a\\_1$。
- $\\texttt{d+abc+abc+abc+abz+abc}$，总得分为 $a\\_1+a\\_1+a\\_1+a\\_1$。
- $\\texttt{d+abcabcabc+abz+abc}$，总得分为 $a\\_3+a\\_1$。
...
- 第四行包含一个由 $m$ 个小写字母组成的字符串。`,
    ...
    memoryLimit: 256,
    hint: `## 样例解释
最优的计分方式为 $\\texttt{d+abc+abc+abc+abz}$，总得分为 $a\\_1+a\\_1+a\\_1$，共 $9$ 分。

## 数据范围

子任务编号|数据点占比|$n$|$m$|$a\\_i$|特殊性质
:-:|:-:|:-:|:-:|:-:|:-:
$1$|$20\\%$|$\\le 20$|$\\le 10^5$|$\\le 1000$|对于所有的 $i(1 \\le i \\le n)$，存在 $a\\_i \\ge a\\_{i+1}$
$2$|$40\\%$|$\\le 3$|$\\le 10^5$|$\\le 1000$|
$3$|$40\\%$|$\\le 20$|$\\le 10^5$|$\\le 1000$|

对于全部数据，保证有 $1\\leq n\\leq 20$，$1\\leq m\\leq 10^5$，$1\\leq a\\_i\\leq 1000$。`,
  },
  {
...
接下来 $q$ 行每行一个正整数 $a\\_i$（$1\\le a\\_i\\le n$），表示第 $i$ 次操作选择的节点编号。`,
...
小杨对于 $m$ 种算法的初始掌握程度均为 $0$。第 $i$ 道题目有对应的知识点 $a\\_i$，即学习第 $i$ 道题目可以令小杨对第 $a\\_i$ 种算法的掌握程度提高 $b\\_i$。小杨的学习目标是对于 $m$ 种算法的掌握程度均至少为 $k$。
...
第二行 $n$ 个正整数 $a\\_1, a\\_2, ..., a\\_n$，代表每道题目的知识点。
第三行 $n$ 个正整数 $b\\_1, b\\_2, ..., b\\_n$，代表每道题目提升的掌握程度。`,
...
对于第 $i$ 个运输站点，其位置为 $p\\_i$ 且至多作为 $c\\_i$ 辆车的初始运输站点。小杨想知道，在最优分配每辆货车的初始运输站点的情况下，所有货车每天的最短总行驶路程是多少。`,
    inputFormat: `第一行包含三个正整数 $n,m,x$，代表运输站点数量、货车数量和两市距离。

之后 $n$ 行，每行包含两个正整数 $p\\_i$ 和 $c\\_i$，代表第 $i$ 个运输站点的位置和最多容纳车辆数。

之后 $m$ 行，每行包含两个正整数 $a\\_i$ 和 $b\\_i$，代表第 $i$ 辆货车每天需要向 A 市运送 $a\\_i$ 次物资，向 B 市运送 $b\\_i$ 次物资。`,
...
| 子任务编号 | 数据点占比 |     $n$     |     $s$     |    $c\\_i$    |
| :--------: | :--------: | :---------: | :---------: | :---------: |
|    $1$     |   $20\\%$   |     $2$     |     $2$     |     $1$     |
|    $2$     |   $20\\%$   | $\\leq 10^5$ | $\\leq 10^5$ |     $1$     |
|    $3$     |   $60\\%$   | $\\leq 10^5$ | $\\leq 10^5$ | $\\leq 10^5$ |

对于全部数据，保证有 $1\\leq n,m\\leq 10^5$，$2\\leq x\\leq 10^8$，$0\\lt p\\_i\\lt x$，$1\\leq c\\_i\\leq 10^5$，$0\\leq a\\_i,b\\_i\\leq 10^5$。数据保证 $\\sum c\\_i\\geq m$。`,
  },
  // ========== 2025年3月 ==========
  {
...
接下来 $n-1$ 行，每行两个整数 $u\\_i,v\\_i$，表示树上有⼀条连接结点 $u\\_i$ 和结点 $v\\_i$ 的边。`,
...
小 A 会从某个车站出发，乘坐地铁环线到某个车站结束行程，这意味着小 A 至少会经过一个车站。小 A 不会经过一个车站多次。当小 A 乘坐地铁环线经过车站 $i$ 时，小 A 会获得 $a\\_i$ 点快乐值。请你安排小 A 的行程，选择出发车站与结束车站，使得获得的快乐值总和最大。`,
    inputFormat: `第一行，一个正整数 $n$，表示车站的数量。

第二行，$n$ 个整数 $a\\_i$，分别表示经过每个车站时获得的快乐值。`,
...
对于所有测试点，保证 $1\\leq n\\leq 2\\times 10^5$，$-10^9\\leq a\\_i\\leq 10^9$。`,
  },
  // ========== 2025年6月 ==========
  {
    ...
    description: `班主任计划将班级里的 $ n $ 名同学划分为若干个学习小组，每名同学都需要分入某一个学习小组中。观察发现，如果一个学习小组中恰好包含 $ k $ 名同学，则该学习小组的讨论积极度为 $ a\\_k $。

给定讨论积极度 $ a\\_1, a\\_2, \\ldots, a\\_n $，请你计算将这 $ n $ 名同学划分为学习小组的所有可能方案中，讨论积极度之和的最大值。`,
    inputFormat: `第一行，一个正整数 $ n $，表示班级人数。

第二行，$ n $ 个非负整数 $ a\\_1, a\\_2, \\ldots, a\\_n $，表示不同人数学习小组的讨论积极度。`,
...
对于所有测试点，保证 $1\\le n\\le 1000$，$0\\le a\\_i\\le 10^4$。`,
  },
  {
...
现在有 $q$ 组询问，第 $i$（$1 \\leq i \\leq q$）组询问给定 $x\\_i, y\\_i$，请你求出编号分别为 $x\\_i, y\\_i$ 的两个结点在这棵树上的距离。两个结点之间的距离是连接这两个结点的简单路径所包含的边数。`,
    inputFormat: `第一行，一个正整数 $q$，表示询问组数。

接下来 $q$ 行，每行两个正整数 $x\\_i, y\\_i$，表示询问结点的编号。`,
    outputFormat: `输出共 $q$ 行，每行一个整数，表示结点 $x\\_i, y\\_i$ 之间的距离。`,
    samples: [
      { input: "3\n1 3\n2 5\n4 8", output: "1\n2\n1" },
      { input: "1\n120 650", output: "9" },
    ],
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $60\\%$ 的测试点，保证 $1 \\leq x\\_i, y\\_i \\leq 1000$。

对于所有测试点，保证 $1 \\leq q \\leq 1000$，$1 \\leq x\\_i, y\\_i \\leq 10^9$。`,
  },
  // ========== 2025年9月 ==========
  {
...
额外地，小 A 还给出了价值 $a\\_1,a\\_2,\\ldots,a\\_n$，表示划分后长度为 $i$ 的子串价值为 $a\\_i$。小 A 希望最大化划分后得到的子串价值之和。你能帮他求出划分后子串价值之和的最大值吗？`,
    inputFormat: `第一行，一个正整数 $n$，表示字符串的长度。

第二行，一个包含 $n$ 个小写字母的字符串 $s$。

第三行，$n$ 个正整数 $a\\_1,a\\_2,\\ldots,a\\_n$，表示不同长度的子串价值。`,
...
对于所有测试点，保证 $1\\le n\\le 10^5$，$1\\le a\\_i\\le 10^9$。`,
  },
  {
    ...
    difficulty: "普及-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1194`,
    description: `A 国有 $n$ 座城市，依次以 $1,2,\\ldots,n$ 编号，其中 $1$ 号城市为首都。这 $n$ 座城市由 $n-1$ 条双向道路连接，第 $i$ 条道路（$1 \\le i < n$）连接编号为 $u\\_i,v\\_i$ 的两座城市，道路长度为 $l\\_i$。任意两座城市间均可通过双向道路到达。
...
接下来 $n-1$ 行，每行三个正整数 $u\\_i,v\\_i,l\\_i$，表示一条双向道路连接编号为 $u\\_i,v\\_i$ 的两座城市，道路长度为 $l\\_i$。`,
...
对于所有测试点，保证 $1 \\le n \\le 10^5$，$1 \\le u\\_i,v\\_i \\le n$，$1 \\le l\\_i \\le 10^9$。`,
  },
  // ========== 2025年12月 ==========
  {
...
初始时 $T$ 中的结点均为白色。你需要将 $T$ 中的若干个结点染为黑色，使得所有叶子到根的路径上至少有一个黑色结点。将结点 $i$ 染为黑色需要代价 $c\\_i$，你需要在满足以上条件的情况下，最小化染色代价之和。

叶子是指 $T$ 中没有子结点的结点。`,
    inputFormat: `第一行，一个正整数 $n$，表示结点数量。

第二行，$n-1$ 个正整数 $f\\_2,f\\_3,\\ldots,f\\_n$，其中 $f\\_i$ 表示结点 $i$ 的父结点的编号，保证 $f\\_i<i$。

第三行，$n$ 个正整数 $c\\_1,c\\_2,\\ldots,c\\_n$，其中 $c\\_i$ 表示将结点 $i$ 染为黑色所需的代价。`,
...
对于另外 $20\\%$ 的测试点，保证 $f\\_i=i-1$。

对于所有测试点，保证 $2\\le n\\le 10^5$，$1\\le c\\_i\\le 10^9$。`,
  },
  {
    ...
    difficulty: "普及/提高-",
    background: `对应的选择、判断题：https://ti.luogu.com.cn/problemset/1202`,
    description: `道具商店里有 $n$ 件道具可供挑选。第 $i$ 件道具可为玩家提升 $a\\_i$ 点攻击力，需要 $c\\_i$ 枚金币才能购买，每件道具只能购买一次。现在你有 $k$ 枚金币，请问你最多可以提升多少点攻击力？`,
    inputFormat: `第一行，两个正整数 $n,k$，表示道具数量以及你所拥有的金币数量。

接下来 $n$ 行，每行两个正整数 $a\\_i,c\\_i$，表示道具所提升的攻击力点数，以及购买所需的金币数量。`,
    ...
    timeLimit: 1000,
    memoryLimit: 256,
    hint: `对于 $60\\%$ 的测试点，保证 $1\\le k\\le 500$，$1\\le c\\_i\\le 500$。

对于所有测试点，保证 $1\\le n\\le 500$，$1\\le k\\le 10^9$，$1\\le a\\_i\\le 500$，$1\\le c\\_i\\le 10^9$。`,
  },
];
