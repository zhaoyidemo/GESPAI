  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3.5 px-5 pt-4">
        ...
        >
          <span className="font-mono text-[8px] opacity-60 tracking-[1px]">LV</span>
          <span className="font-mono text-lg font-bold leading-none" style={{ color: theme.nameColor }}>
            {boss.level}
          </span>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2.5 flex-wrap">
            <span className="font-mono text-[22px] font-bold" style={{ color: theme.nameColor }}>
              {boss.name}
            </span>
            <span className="text-[14px] text-[#6b7280] font-light">{boss.subtitle}</span>
          </div>
          <div className="flex gap-1.5 mt-1 flex-wrap">
            {boss.badges.map((badge, i) => {
              ...
              return (
                <span
                  key={i}
                  className="text-xs px-2 py-[2px] rounded font-semibold tracking-[0.5px] whitespace-nowrap uppercase"
                  style={{
      ...
      >
        ...
        <div className="bg-[#0d1220] px-2.5 py-2 flex flex-col gap-[2px]">
          <span className="text-xs text-[#4a5060] uppercase tracking-[0.8px] font-medium">难度</span>
          <StarRating count={boss.difficulty} />
        </div>
        {/* 其他数据 */}
        {boss.stats.map((stat, i) => (
          <div key={i} className="bg-[#0d1220] px-2.5 py-2 flex flex-col gap-[2px]">
            <span className="text-xs text-[#4a5060] uppercase tracking-[0.8px] font-medium">{stat.label}</span>
            <span
              className="font-mono text-[16px] font-semibold"
              style={{
                color: stat.color || "#e2e5ed",
                fontSize: stat.small ? "14px" : undefined,
              }}
            >
              {stat.value}
            </span>
      ...
      <div className="flex gap-1.5 flex-wrap px-5 py-2 pt-2 pb-2.5">
        {boss.loot.map((item, i) => {
          ...
          return (
            <span
              key={i}
              className="text-[13px] px-2 py-[3px] rounded flex items-center gap-1 font-medium"
              style={{
                background: colors.bg,
                color: colors.text,
                border: `1px solid ${colors.border}`,
      ...
      {/* 说明文字 */}
      <div className="px-5 py-2.5 pb-3.5 text-[15px] text-[#6b7280] leading-[1.7] border-t border-white/[0.06]">
  ...
  return (
    <div>
      <div className="flex items-center gap-2.5 py-1.5 mx-auto max-w-[480px]">
        <span className="text-[13px] text-[#4a5060] whitespace-nowrap font-mono min-w-[130px] text-right">
          {bridge.label}
        </span>
        ...
        <span className="text-[13px] font-mono font-semibold min-w-[52px]" style={{ color: bridge.rateColor }}>
          {bridge.rate}
        </span>
      </div>
      {bridge.note && (
        <div className="max-w-[480px] mx-auto px-0 pb-1 text-[13px] text-[#4a5060] leading-[1.6] text-center">
          {bridge.note}
        </div>
      )}
    </div>
  ...
  return (
    <div className="bg-white/[0.025] border border-white/[0.06] rounded-lg p-3.5 px-4 relative transition-all duration-200 hover:bg-white/[0.05] hover:border-white/10">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="font-mono text-[15px] font-bold" style={{ color: quest.nameColor }}>
          {quest.name}
        </span>
        <span className="font-mono text-xs text-[#4a5060] ml-auto uppercase tracking-[0.5px]">
          {quest.levelTag}
        </span>
      </div>
      <p className="text-[14px] text-[#6b7280] leading-[1.7]">{quest.description}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#080c14] text-[#e2e5ed]" style={{ fontFamily: "'Noto Sans SC', 'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      <div className="max-w-[900px] mx-auto px-5 pt-28 pb-20 sm:pt-32">
        ...
        <header className="text-center mb-14 relative">
          <div className="inline-block font-mono text-xs tracking-[2px] text-amber-400/70 mb-3">
            ◆ QUEST MAP ◆
          </div>
          <h1
            className="text-[34px] font-black tracking-[-0.5px] mb-2.5"
            style={{
          ...
          <div className="text-[15px] text-[#4a5060] font-light">
        ...
        <div className="mt-12 pt-7 border-t border-white/[0.06]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="text-base font-bold text-[#6b7280] tracking-[1.5px] uppercase font-mono">
              支线副本 · Side Quests
            </span>
          </div>
        ...
        {/* ═══ 数据来源 Footer ═══ */}
        <div className="mt-12 pt-5 border-t border-white/[0.06] text-[13px] text-[#4a5060] leading-[1.8]">
          <p>
